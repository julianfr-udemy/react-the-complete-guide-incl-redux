...
export default (state = { ingredients: null, totalPrice: 4, error: false }, action) => {
  switch (action.type) {
    case ADD:
      const ingredients = updateObject(state.ingredients, { [action.ingredient]: state.ingredients[action.ingredient] + 1 });
      return updateObject(state, { totalPrice: state.totalPrice + INGREDIENT_PRICES[action.ingredient], ingredients });
    case REMOVE:
